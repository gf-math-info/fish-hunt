...
public class PlanJeu {
    ...
    private final double POISSON_GRANDEUR_MIN = 30;
    private final double POISSON_GRANDEUR_MAX = 50;
    ...
    private void ajouterBulles() {
        double posGroupe, posBulle, rayonBulle, vitesseBulle;
        for(int i = 0; i < NB_GROUPES_BULLES; i++) {
            ...
            for(int j = 0; j < NB_BULLES; j++) {
                ...
                rayonBulle = random.nextDouble() *
                        (BULLE_RAYON_MAX - BULLE_RAYON_MIN) +
                        BULLE_RAYON_MIN;
                vitesseBulle = random.nextDouble() *
                        (BULLE_VITESSE_MAX - BULLE_VITESSE_MIN) +
                        BULLE_VITESSE_MIN;
                bulles.add(new Bulle(rayonBulle,
                        posBulle, hauteur + rayonBulle, vitesseBulle));
    ...
    private void ajouterPoissonSpecial() {
        ...
        if(random.nextBoolean()) {//Un crabe...
            ...
            double vx = VITESSE_CRABE * partie.getNiveau();
            poissons.add(new Crabe(largeurPoisson, hauteurPoisson, x, y, vx));
